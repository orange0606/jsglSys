        ...
        <el-row :gutter="0">
            ...
            <el-col :span="5" :offset="1" :xs="23" :sm="13" :md="8" :lg="6" :xl="5">
                <div class="headerform">
                    ...
                    <el-form :model="Form" v-show="!showAtt" :rules="rules" ref="Form" label-width="90px" size="small" class="demo-ruleForm">
                       ...
                       <el-form-item v-if="Form.type == 'totalpay'" label="选择支付表头" prop="tTotalmeterageHeadId">
                            <el-select v-model="Form.payHeadId" placeholder="请选择支付清单清单表头"  clearable size="small" style=" width:100%;">
                                <el-option v-for="(val,i) in payHeadList" :key="i+'e'" :label="val.name" :value="val.id"></el-option>
                            </el-select>
                        </el-form-item>
                    ...
                    <el-form v-show="showAtt" :model="row" ref="row" :rules="rules" label-width="100" width="800" size="small">
    ...
    methods: {
        ...
        typeChange (req) {  //选择表头的类型
           ...
           if (req == 'change') {
                ...
                if (req == 'totalpay') {
                    this.allpay();
                    return this.Form.tOriginalHeadId = this.Form.tMeterageHeadId = this.Form.tUpdateHeadId = null;

                }
                this.Form.payHeadId = this.Form.tOriginalHeadId = this.Form.tMeterageHeadId = this.Form.tUpdateHeadId = null;
            }
        },
        ...
        initial () {    //提交完成与取消按钮的初始化
            ...
            //利用引用赋值给新建表头表单子组件进行赋值
            this.Form.sysOrder = null;          //系统序号 预留，暂时不用
            this.Form.sysNum = null;           //系统编号 预留，暂时不用
            this.Form.tenderId = null;   //标段id 
            this.Form.num = null;    //表头编号
            this.Form.name = null;           //表名
            this.Form.type = null;          //类别 original原清单change变更清单update变更后的清单meterage计量清单 totalmeterage累计计量清单 pay支付清单 totalpay累计支付清单
        ...
        attChange (type) {  //选择属性的选择框改变事件
            ...
            this.row.attributePayHeadRowId = null;
