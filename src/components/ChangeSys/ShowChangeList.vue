<template>
    ...
    <div v-loading="loading" element-loading-text="飞速加载中">
       ...
       <!-- <elx-editable-column prop="id" label="ID" width="80"></elx-editable-column> -->
        ...
        <elx-editable-column prop="name" label="变更清单名称" min-width="110" align="center" fixed="left" show-overflow-tooltip :edit-render="{name: 'ElInput'}" ></elx-editable-column>
        ...
        <transition name="el-fade-in">
          <el-dialog :title="EditTitle" width="85%" top="4vh" custom-class="dialogs" :lock-scroll="false" :visible.sync="visibleNew">
              <show-new-change :tender="tender" :refresh.sync="visibleNew" :uplist="uprow" :approval="approval" ></show-new-change>
              <br><br><br>
          </el-dialog>
        </transition>
    </div>
...
<script>
import ShowNewChange from './ShowNewChange';
import XEUtils from 'xe-utils'
  ...
  props: {
    ...
    approval:{
      type: Object,
      required: false,
      default: () => ({id:172, name:"变更审批单3-zfh2-变更900-4",state: 0}) //state=1为已通过的审批单
    },
    tender:{
      type: Object,
      required: false,
       ...
       this.$confirm('确定删除所选数据?', '温馨提示', {
          ...
          type: 'warning'
            ...
            for (let index = 0; index < removeRecords.length; index++) {
                changeIdList.push(removeRecords[index].id)
            }
            // 进行发起请求删除
            this.$post('/change/delarray',{ changeIdList })
              .then((response) => {
              //删除成功
              this.loading = false
              this.findList()
      ...
      this.$refs.elxEditable.validateRow(row, valid => {
        if (valid) {
            var obj = {
                ...
                name: row.name,                     //原清单名称
                num: row.num,                    //原清单编号
                changeRowList: null                 //原清单内容，如果为null表示无内容修改，如果为空数组，表示删除全部内容
            },
            changeList = [],
            url = '/change/update';
            changeList.push(obj);
