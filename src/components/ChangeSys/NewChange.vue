 ...
 watch: {
  ...
  methods: {
    ...
    RowCss({row, rowIndex}) {     // 定义RowCss函数，这样当表格中的相应行满足自己设定的条件是就可以将该行css样式改变
        if (row['A'].attribute && row['A'].attribute === 'add') {
    ...
    updates (row) {  //新建模式与修改模式的预览修改数据呈现函数
          ...
          try {
              this.list = this.$excel.ListAssemble(row.changeRowList); //组装清单表格数据;
    ...
    oneHeader (id) {  //请求单个表头 表头id  表头类型
     ...
     OneChange (id) { //变更清单id
        ...
        this.$post('/change/row/getone',{ id })
            ...
            this.loading = false;
            this.list = this.$excel.ListAssemble(data.changeRowList); //组装清单表格数据;
            this.findList(); //调用滚动渲染数据
            this.ResetList = XEUtils.clone(this.list, true); //深拷贝 用来重置使用
